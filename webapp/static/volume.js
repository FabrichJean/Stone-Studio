...
const segmentVolume = document.getElementById("segmentVolume");
const segmentVolumeValue = document.getElementById("segmentVolumeValue");
...
modeToggle.querySelectorAll(".mode-toggle-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    setPreviewVolume(mode === "global" ? parseFloat(globalVolume.value) / 100 : 1);
  });
});

/* ---------- Réglages de volume ---------- */

// <video>.volume est plafonné à 1 (100%) par le navigateur : impossible d'amplifier au-delà
// de l'original par ce biais. On route donc l'élément via un GainNode (Web Audio API), dont
// le gain n'a pas cette limite, pour que l'aperçu reflète fidèlement une amplification.
let audioCtx = null;
let gainNode = null;
...
function setPreviewVolume(factor) {
  ensureAudioGraph();
...
globalVolume.addEventListener("input", () => {
  globalVolumeValue.textContent = `${globalVolume.value} %`;
  if (mode === "global") setPreviewVolume(parseFloat(globalVolume.value) / 100);
});

segmentVolume.addEventListener("input", () => {
  segmentVolumeValue.textContent = `${segmentVolume.value} %`;
});
    ...
    .map(
      ...
      <div class="segment-item" data-index="${i}" title="Cliquer pour prévisualiser ce morceau">
        <span>
          ...
          <span class="segment-speed-tag">${seg.factor * 100}%</span>
        </span>
...
previewBtn.addEventListener("click", () => {
  ...
  setPreviewVolume(parseFloat(segmentVolume.value) / 100);
  preview.play();
...
addSegmentBtn.addEventListener("click", () => {
  ...
  segments.push({ start: startTime, end: endTime, factor: parseFloat(segmentVolume.value) / 100 });
...
function setVolumeProgress(percent, label) {
  progressWrap.hidden = false;
  progressFill.style.width = `${percent}%`;
  progressLabel.textContent = label;
}

function pollVolumeProgress(jobId) {
  return new Promise((resolve, reject) => {
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`/api/volume-media/${jobId}/progress`);
        ...
        if (job.status === "processing") {
          ...
          setVolumeProgress(job.percent, label);
        } else if (job.status === "done") {
          clearInterval(interval);
          setVolumeProgress(100, "Traitement terminé.");
          resolve(job);
...
applyBtn.addEventListener("click", async () => {
  ...
  status.className = "status";
  status.textContent = "";
  setVolumeProgress(0, "Démarrage du traitement...");
  ...
  if (mode === "global") {
    formData.append("factor", parseFloat(globalVolume.value) / 100);
  } else {
    formData.append(
      "segments",
      JSON.stringify(
  ...
  try {
    const startRes = await fetch("/api/volume-media", { method: "POST", body: formData });
    if (!startRes.ok) {
    ...
    const job = await pollVolumeProgress(job_id);

    pendingDownload = { projectId: job.project_id, name: job.output_name };
    downloadBtn.hidden = false;
    renderSendTo("sendToWrap", job.project_id, "volume_media");

    status.textContent = `Volume ajusté avec succès (${formatBytesCommon(job.output_size)}).`;
